...
class IdiomService {
  ...
  async initialize(csvUrl: string): Promise<void> {
    try {
      const { data } = await Taro.request({ url: csvUrl })
      this.parseCSV(data)
      this.buildMaps()
    } catch (error) {
   ...
   * @returns 满足条件的成语数组
   */
  getNextIdioms(idiom: Idiom, limit: number = 10, excludes: string[] = []): Idiom[] {
    console.log({
      idiom: idiom,
      limit: limit,
    })
  ...
  getSuggestions(prefix: string, limit: number = 10): Idiom[] {
    const suggestions = this.idioms.filter(idiom => idiom.word.includes(prefix)).slice(0, limit)
    return suggestions
  }
}
